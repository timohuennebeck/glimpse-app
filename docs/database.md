...
1. Every upload writes **two objects** to the private `moments` bucket:
   - `original/{user}/{ts}.jpg`
   - `blurred/{user}/{ts}.jpg` (heavy Gaussian, generated by an Edge Function on
...
2. The bucket has **no public access**, and the **RLS policy on `storage.objects`**
   is the lock: it allows `SELECT` on the original only for the author or a party
   to an *open* trade, and on the blurred copy only for a party to *any* trade.
